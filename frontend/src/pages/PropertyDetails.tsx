...
import { 
  Bed, Bath, Maximize, MapPin, Building2, ArrowLeft, Phone, 
  MessageCircle, Calendar, Home, Car, Utensils, Layers, 
  ChevronLeft, ChevronRight, CheckCircle2, PlayCircle, FileText, X
} from "lucide-react";
      ...
      try {
        ...
        if (data && data.property) {
          ...
          const mapped: PropertyType = {
            ...
            category: p.listedIn === "Off-Plan" ? "off-plan" : (p.listedIn === "Rent" ? "rental" : "secondary"),
            status: p.status === "active" ? "ready" : "off-plan",
            image: p.carousel_thumb?.[0]?.img || "/images/property-placeholder.jpg",
        ...
        <div className="relative h-[60vh] md:h-[75vh] w-full bg-black overflow-hidden group">
          <AnimatePresence mode="wait">
            <motion.img
              ...
              onClick={() => setPreviewImage(property.images?.[activeImage] || property.image)}
            />
          </AnimatePresence>
          ...
          <div className="absolute top-10 left-6 flex items-center gap-4">
            <Button variant="hero-outline" size="sm" asChild className="backdrop-blur-md bg-white/5 border-white/20">
              <Link to="/properties">
              ...
              >
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                  ...
                  <div className="text-left md:text-right">
                    <p className="text-[10px] font-black text-gold uppercase tracking-[0.3em] mb-2">Asking Price</p>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-8 bg-muted/30 rounded-3xl border border-border/50">
                  <div className="flex flex-col items-center">
                    ...
                    <p className="font-display text-xl font-black text-foreground">{property.bedrooms}</p>
                    <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Bedrooms</p>
                  </div>
                  <div className="flex flex-col items-center border-l border-border/50">
                    <div className="w-12 h-12 rounded-2xl bg-background flex items-center justify-center mb-3 shadow-inner">
                      <Bath className="w-5 h-5 text-gold" />
                    </div>
                    <p className="font-display text-xl font-black text-foreground">{property.bathrooms}</p>
                    <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Bathrooms</p>
                  </div>
                  <div className="flex flex-col items-center border-l border-border/50">
                    <div className="w-12 h-12 rounded-2xl bg-background flex items-center justify-center mb-3 shadow-inner">
                      <Maximize className="w-5 h-5 text-gold" />
                    </div>
                    <p className="font-display text-xl font-black text-foreground">{property.area.toLocaleString()}</p>
                    <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Sq. Ft.</p>
                  </div>
                  <div className="flex flex-col items-center border-l border-border/50">
                    <div className="w-12 h-12 rounded-2xl bg-background flex items-center justify-center mb-3 shadow-inner">
                      <Home className="w-5 h-5 text-gold" />
                    </div>
                    <p className="font-display text-base font-black text-foreground capitalize">{property.type}</p>
                ...
                <div className="mt-12">
                  ...
                  <div className="prose prose-stone max-w-none">
                    <p className="text-muted-foreground font-body leading-[1.8] text-base whitespace-pre-line">
                      {property.description || `Experience high-end urban living in this pristine ${property.type}. Located in the heart of ${property.location}, this residence combines architectural excellence with panoramic views.`}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-12 pt-12 border-t border-border">
                   <h2 className="font-display text-2xl font-black text-foreground mb-8 uppercase tracking-tighter">Technical Profile</h2>
                   <div className="grid grid-cols-2 md:grid-cols-3 gap-8">
                      {property.yearBuilt && (
                        <div className="flex items-center gap-4">
                           <Calendar className="w-5 h-5 text-gold/50" />
                           <div>
                              <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Built Year</p>
                              <p className="text-sm font-bold">{property.yearBuilt}</p>
                           </div>
                        </div>
                      )}
                      {property.kitchens && (
                        <div className="flex items-center gap-4">
                           ...
                           <div>
                              <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Kitchens</p>
                              <p className="text-sm font-bold">{property.kitchens}</p>
                           </div>
                        </div>
                      )}
                      {property.garages && (
                        <div className="flex items-center gap-4">
                           ...
                           <div>
                              <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Parking</p>
                              <p className="text-sm font-bold">{property.garages} {property.garageSize ? `(${property.garageSize} sqft)` : 'Spaces'}</p>
                           </div>
                        </div>
                      )}
                      {property.floorsNo && (
                        <div className="flex items-center gap-4">
                           ...
                           <div>
                              <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Total Floors</p>
                              <p className="text-sm font-bold">{property.floorsNo}</p>
                           </div>
                        </div>
                      )}
                   </div>
                ...
                {property.amenities && property.amenities.length > 0 && (
                  <div className="mt-12 pt-12 border-t border-border">
                    <h2 className="font-display text-2xl font-black text-foreground mb-8 uppercase tracking-tighter">Lifestyle Amenities</h2>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {property.amenities.map((amenity, idx) => (
                        <div key={idx} className="flex items-center gap-3 p-3 rounded-xl bg-muted/20 border border-border/30">
                          <CheckCircle2 className="w-4 h-4 text-emerald shrink-0" />
                          <span className="text-xs font-bold text-foreground/80 tracking-tight">{amenity}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                      ...
                      {property.floorPlans.map((plan, idx) => (
                        ...
                        >
                           ...
                           <div className="absolute inset-0 bg-primary/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white">
                              ...
                              <span className="text-[10px] font-black uppercase tracking-widest text-center px-4 leading-normal">Expand Blueprint {idx + 1}</span>
                           </div>
                           <div className="absolute top-4 right-4 bg-primary/80 backdrop-blur-sm px-3 py-1.5 rounded-full border border-white/10">
                              <span className="text-[10px] font-black text-white uppercase tracking-widest group-hover:text-gold transition-colors">Plan {idx + 1}</span>
                           </div>
                        </div>
                      ))}
                   </div>
                   <div className="mt-8 pt-8 border-t border-border/50 text-center">
                      <p className="text-muted-foreground text-xs italic mb-4">Detailed technical specifications and high-resolution CAD exports are available upon request.</p>
                      <Button variant="outline" className="rounded-full px-8" asChild>
                         <a href={property.floorPlans[0]} target="_blank" rel="noreferrer">Download High-Res PDF</a>
                      </Button>
                   </div>
                </div>
              )}
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-32 space-y-6">
                <div className="bg-background rounded-3xl p-8 border border-border shadow-2xl relative overflow-hidden group">
                  ...
                  <h3 className="font-display font-black text-xs uppercase tracking-[0.3em] text-gold mb-6">Expert Consultation</h3>
                  ...
                  {property.developer && (
                    <div className="flex items-center gap-4 mb-8 pb-8 border-b border-border">
                       ...
                       <div>
                          <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Portfolio Manager</p>
                          <p className="font-bold text-foreground">{property.developer}</p>
                       </div>
                    </div>
                  )}

                  <div className="space-y-4">
                    <Button variant="gold" size="xl" className="w-full rounded-2xl py-8 shadow-xl shadow-gold/20" asChild>
                      <Link to="/contact">
                        <MessageCircle className="w-5 h-5 mr-3" /> Get Official Brochure
                      </Link>
                    </Button>
                    <Button variant="hero-outline" size="xl" className="w-full rounded-2xl py-8 border-2" asChild>
                      <Link to="/contact">
                        <Phone className="w-5 h-5 mr-3" /> Schedule Private Viewing
                      </Link>
                    </Button>
                    
                    <div className="pt-6 flex justify-center gap-6">
                       <a href="https://wa.me/971000000000" target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-emerald transition-colors">
                          <MessageCircle className="w-6 h-6" />
                       </a>
                       <div className="w-px h-6 bg-border" />
                       <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest py-1">Share Asset</span>
                    </div>
                  </div>
                </div>
                ...
                {property.videoUrl && (
                  <div className="bg-primary text-white rounded-3xl p-8 flex flex-col items-center text-center shadow-2xl shadow-primary/30 group">
                    ...
                    <h3 className="font-display font-black text-lg uppercase mb-2">Cinematic Video Tour</h3>
                    <p className="text-white/60 text-xs mb-6 px-4">Experience the spatial atmosphere through our professional 4K virtual walkthrough.</p>
                    <Button variant="gold" className="rounded-full px-10 py-6" asChild>
                      <a href={property.videoUrl} target="_blank" rel="noreferrer">Enter Virtual Experience</a>
                    </Button>
                  </div>
                )}
                ...
                <div className="bg-primary/5 rounded-3xl p-8 border border-gold/10">
                   ...
                   <h4 className="font-display font-black text-sm uppercase mb-2">Investment Security</h4>
                   <p className="text-[11px] text-muted-foreground leading-relaxed">
                     All transfers in the Dubai Dreams showcase are handled via RERA regulated Escrow accounts ensuring 100% investor protection.
                   </p>
                </div>
              </div>
            </div>
               ...
               >
                  ...
                  {property.floorPlans?.includes(previewImage) && (
                    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-gold/90 backdrop-blur-md px-6 py-2 rounded-full">
                       <span className="text-[10px] font-black text-accent-foreground uppercase tracking-[0.2em]">Technical Blueprint View</span>
                    </div>
                  )}
               </motion.div>
            </motion.div>
